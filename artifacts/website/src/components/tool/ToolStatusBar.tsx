...
export function ToolStatusBar({
  ...
  // Toggle body class so that toast styles can adjust position when the status bar is active
  useEffect(() => {
    if (!hidden) {
      document.body.classList.add("tool-status-bar-active");
    } else {
      document.body.classList.remove("tool-status-bar-active");
    }
    return () => {
      document.body.classList.remove("tool-status-bar-active");
    };
