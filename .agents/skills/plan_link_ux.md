...
This skill creates a structured, phased plan to implement the pointer-events bypass, a custom link tooltip BubbleMenu, and popover link editing inputs, removing the primitive `window.prompt` blocker.
...
2. **Define Phases**:
   ...
   - **Phase 2: Floating Tooltip BubbleMenu (Desktop)**
     - Objective: Add a floating action panel when cursor is inside a link in `App.tsx`.
     - Sub-steps:
       ...
       - 2.2: Set the BubbleMenu trigger rule: `shouldShow={({ editor }) => editor.isFocused && editor.isActive("link")}`.
   - **Phase 3: Floating Tooltip BubbleMenu (Web)**
     ...
     - Sub-steps:
       ...
       - 3.2: Replicate the Link BubbleMenu markup, styling, and handlers in `notepad.tsx`.
